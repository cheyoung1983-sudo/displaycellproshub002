...
// Single query execution on primary cluster with automatic retry on transient connection drops
export async function query(sql: string, args: unknown[] = [], retries = 2): Promise<any> {
  totalQueriesExecuted++;
  const pool = getDatabasePool();

  // Guard against queue explosion under massive burst concurrency
...
// Single query execution on read-only cluster replica with fallback and retry
...
/**
 * Migration DDL Script: Adds B-Tree indexes on 'device_model' and 'repair_category'
 * columns in the 'supported_devices' table to accelerate Repair Portal lookups.
 */
export const SUPPORTED_DEVICES_INDEX_MIGRATION_SQL = `
-- 1. Ensure supported_devices table exists
...
-- Ensure target columns exist if table was already present with older schema
...
-- 2. B-Tree Indexes on high-traffic search columns for Repair Portal
...
/**
 * Executes migration script that adds B-tree indexes to 'device_model' and 'repair_category'
 * columns in the 'supported_devices' table to boost lookup performance for the Repair Portal.
 */
export async function runSupportedDevicesIndexMigration(): Promise<MigrationResult> {
  ...
  try {
    // Execute DDL statements on primary database pool
    await query(SUPPORTED_DEVICES_INDEX_MIGRATION_SQL, []);
    console.log('[Migration] B-tree indexes on supported_devices (device_model, repair_category) successfully applied.');
